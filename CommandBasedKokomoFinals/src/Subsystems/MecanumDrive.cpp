...
void MecanumDrive::Drive(float x, float y, float z, float angle)
{
	...
	SmartDashboard::PutNumber(T_LIDAR_DISTANCE_ONE, lidarValueOne);
...
void MecanumDrive::GetSensorValues()
{
	...
	//int oldValue = lidarValueOne;
	//lidarValueOne = lidar->GetValue();

	//SmartDashboard::PutNumber("LIDAR Jump", fmax(SmartDashboard::GetNumber("LIDAR Jump", 0), lidarValueOne - oldValue));
}
