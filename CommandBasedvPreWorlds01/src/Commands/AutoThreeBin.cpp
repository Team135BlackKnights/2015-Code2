...
AutoThreeBin::AutoThreeBin(bool ramp)
{
	...
	AddParallel(new ExternalMoveWinch(-.2, 10.0f));
	...
	AddSequential(new AutoMoveRobot(.8f * dirMult, 0, DISTANCE_CLOSE_TO_WALL, AutoMoveRobot::MODE::DISTANCE, true, 6.0f));
	AddSequential(new AutoMoveRobot(.1f, .55f, 1.35f, AutoMoveRobot::MODE::TIME, true, 1.0f));
	...
	AddSequential(new AutoMoveRobot(-.7f * dirMult, -.05f, DISTANCE_END, AutoMoveRobot::MODE::DISTANCE, true, 10.0f));
	//AddSequential(new AutoMoveRobot(0,  0, 2.0f, AutoMoveRobot::TIME, true));

	//Drop Internal
	//while (DriverStation::GetInstance()->GetMatchTime() > 137) {};
	//AddSequential(new InternalSolenoidRoller(InternalCollect::ROLLER_COLLECT_DISENGAGED));
